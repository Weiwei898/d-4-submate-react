...
// 設定 API 基礎路徑 (針對 users 資料表)
const BASE_URL = `${SUPABASE_URL}/rest/v1/users`;
...
export const getUsers = async () => {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.message || '無法取得使用者列表');
  }
...
export const getUserById = async (id) => {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.message || '無法取得該使用者');
  }
...
export const deleteUser = async (id) => {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.message || '刪除使用者失敗');
  }
...
export const updateUser = async (id, updates) => {
  ...
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.message || '更新使用者失敗');
  }
